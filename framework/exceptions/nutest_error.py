...
from framework.lib.nulog import ERROR, WARN
from framework.lib.exception_decoder import ExceptionDecoder
...
class NuTestError(Exception):
  ...
  def __new__(cls, message='', collector=None, **kwargs):
    ...
    if mod_path in exception_files and not is_nutest_child:
      ...
      return super(NuTestError, cls).__new__(cls, message, collector, **kwargs)
  ...
  def __init__(self, message='', collector=None, mute_exception_warning=False,
    ...
    super(NuTestError, self).__init__(message)
