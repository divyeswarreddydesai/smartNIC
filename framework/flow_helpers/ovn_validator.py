...
class OvnValidator:
    ...
    def get_ip_of_vm(self,vm_name):
        ...
        for nic in vm_data["nics"]:
            if "networking_info" in nic.keys():
                try:
                    vm_ip=nic['network_info']['ipv4_config']['ip_address']['value']
                except KeyError as e:
                    ERROR(f"Failed to get VM IP: {e}")
                    vm_ip=None
        return vm_ip
